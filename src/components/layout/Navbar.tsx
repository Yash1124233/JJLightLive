  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[var(--lighting-border)]/60 bg-[var(--lighting-bg)]/80 backdrop-blur-xl transition-colors duration-700">
      <div className="mx-auto flex h-[82px] max-w-[1440px] items-center justify-between px-6 lg:px-12">
        <Link
          to="/"
          aria-label="JJ A Lighting Co. home"
          className="inline-flex items-center rounded-md px-3 py-1.5 transition-colors duration-700 sm:px-3.5 sm:py-2"
        >
          <img
            src="/images/logo.png"
            alt="JJ A Lighting Co."
          ...
          {navLinks.map((link) =>
            link.href.startsWith("#") ? (
              <a
                ...
                className="label-uppercase text-[var(--lighting-muted)] transition-colors hover:text-[var(--lighting-text)]"
              >
                {link.label}
              </a>
            ) : (
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-2 text-[var(--lighting-text)] lg:hidden"
      ...
      {menuOpen && (
        <nav className="border-t border-[var(--lighting-border)] bg-[var(--lighting-bg)] px-6 py-5 lg:hidden">
          {navLinks.map((link) =>
            link.href.startsWith("#") ? (
              <a
                key={link.label}
                href={link.href}
                onClick={(event) => handleAnchor(event, link.href)}
                className="block py-3 label-uppercase text-[var(--lighting-muted)]"
              >
                {link.label}
              </a>
            ) : (
